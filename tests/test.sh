...
assert_file "$FAKE_PROFILE/chrome/JS/Omazen/omazen-chrome-v0.1.3.css"
assert_file "$FAKE_PROFILE/chrome/JS/Omazen/omazen-content-v0.1.3.css"
assert_file "$FAKE_HOOKS/theme-set.d/theme-set"
grep -Fq 'Omazen: 0.1.3' <(run_omazen status) || fail "reported package version"
grep -Fq 'omazen-chrome-v0.1.3.css' "$PROJECT_ROOT/zen/omazen-bridge.uc.js" || \
  fail "chrome stylesheet cache buster"
grep -Fq 'omazen-content-v0.1.3.css' "$PROJECT_ROOT/zen/Omazen/OmazenChild.sys.mjs" || \
...
grep -Fq -- '--zen-urlbar-background-base: var(--omazen-background-light)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css" || fail "inactive URL bar background"
grep -Fq -- '--lwt-toolbar-field-focus: var(--omazen-background-light)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css" || fail "focused URL bar background"
grep -Fq -- '--zen-urlbar-background-transparent: var(--omazen-background-light)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css" || fail "expanded URL bar background"
grep -Fq -- '#urlbar:is([focused="true"], [breakout-extend]) .urlbar-background' \
  "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css" || fail "focused URL bar outline"
if grep -Fq -- '#urlbar-background' "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css"; then
  fail "obsolete URL bar background ID selector"
fi
if grep -Fq -- 'zen-workspace[active]' "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css"; then
  fail "active workspace container must not receive selection background"
fi
if grep -Fq -- '.zen-current-workspace-indicator' "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css"; then
  fail "workspace indicator must retain native padding and background"
fi
if sed -n '/:is(/,/)/p' "$PROJECT_ROOT/zen/Omazen/omazen-chrome-v0.1.3.css" | grep -Fxq '  input'; then
  fail "generic input selector must not repaint the URL text field"
fi
grep -Fq -- '--background-color-canvas: var(--omazen-background)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-content-v0.1.3.css" || fail "Settings canvas palette"
grep -Fq -- '--input-text-background-color: var(--omazen-background-dark)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-content-v0.1.3.css" || fail "Settings search palette"
grep -Fq -- '--theme-bg-color: var(--omazen-background-light)' \
  "$PROJECT_ROOT/zen/Omazen/omazen-content-v0.1.3.css" || fail "managed notice palette"
